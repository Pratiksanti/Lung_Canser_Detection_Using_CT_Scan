...
async function searchHospitalByName(query) {
  ...
  return data
    ...
    .map((r) => ({
      ...
      address:     [r.address?.road, r.address?.city || r.address?.town, r.address?.state]
                     .filter(Boolean).join(", "),
...
function HospitalMap() {
  ...
  const runSearch = useCallback(async (q) => {
    if (!q.trim() || q.trim().length < 2) { setSearchResults([]); setShowDropdown(false); return; }
    ...
    try {
      ...
      if (results.length === 0) setSearchError("No hospitals found. Try a different name or city.");
    } catch {
  ...
  const fetchNearbyHospitals = async (lat, lon, radiusKm = 10) => {
    setStatus("loading");
    ...
    try {
      ...
      throw new Error("Could not load nearby hospitals. Please try again.");
    }
  };
  ...
  const handleGetDirections = async (hospital, e) => {
    ...
    if (!loc) {
      setErrorMsg("Please click 'Find Hospitals Near Me' first to detect your location.");
    ...
    try {
      const { coordinates, distanceKm, durationMin } = await fetchOSRMRoute(
        loc.lat, loc.lon, hospital.lat, hospital.lon
      );
      ...
      setTimeout(() => setErrorMsg(""), 4000);
      openExternalDirections(loc.lat, loc.lon, hospital.lat, hospital.lon, hospital.name);
    } finally {
  ...
  const addNearbyMarkersToMap = (results, userLat, userLon) => {
    ...
    L.circle([userLat, userLon], {
      radius: 10000, color: "#2563eb", fillColor: "#3b82f6",
      fillOpacity: 0.05, weight: 1.5, dashArray: "6 4",
    }).addTo(map);

    results.forEach((hospital) => {
      const marker = L.marker([hospital.lat, hospital.lon], { icon: HOSPITAL_ICON })
        .addTo(map).bindPopup(buildPopupHTML(hospital));
      markersRef.current[hospital.id] = marker;
    });

    if (results.length > 0) {
      map.fitBounds([[userLat, userLon], ...results.map((h) => [h.lat, h.lon])], { padding: [40, 40] });
    } else {
  ...
  const allHospitals = [...hospitals, ...(selectedSearch ? [selectedSearch] : [])];
  ...
  useEffect(() => {
    ...
    window.__openExternal = (id) => {
      const h = allHospitals.find((r) => String(r.id) === String(id));
      if (h && userLocation)
        openExternalDirections(userLocation.lat, userLocation.lon, h.lat, h.lon, h.name);
    };
  ...
  const handleLocate = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        try {
          const results = await fetchNearbyHospitals(lat, lon, 10);
  ...
  return (
    <div className="hospital-map-page">
      ...
      <div className="map-header">
        ...
        <p>Detect hospitals near you, or search for any specific hospital by name.</p>
        ...
        >
          {status === "locating" ? "Getting your location..."
            : status === "loading" ? "Finding hospitals..."
            : "📍 Find Hospitals Near Me"}
        </button>

        {status === "error"  && <div className="map-error">⚠️ {errorMsg}</div>}
        {errorMsg && status !== "error" && <div className="map-error">⚠️ {errorMsg}</div>}
        {status === "done"   && (
          <div className="map-success">
            ✅ Found <strong>{hospitals.length}</strong> hospital{hospitals.length !== 1 ? "s" : ""} within 10 km.
          </div>
        )}
      </div>
              ...
              {searchResults.map((r) => (
                ...
                >
                  ...
                  <div>
                    ...
                    <span className="dropdown-addr">{r.address || r.fullName.split(",").slice(1, 3).join(",")}</span>
                  </div>
                </li>
              ))}
            </ul>
        ...
        {selectedSearch && (
          <div className="selected-hospital-card">
            <div className="selected-left">
              ...
              {selectedSearch.address && <span className="selected-addr">{selectedSearch.address}</span>}
            </div>
            <div className="selected-actions">
              ...
              >
                {routeLoadingId === selectedSearch.id ? "Loading..." : "🗺️ Get Directions"}
              </button>
              {userLocation && (
                <button
                  ...
                  onClick={() => openExternalDirections(
                    ...
                    selectedSearch.lat, selectedSearch.lon, selectedSearch.name
                  )}
                >↗ Maps</button>
              )}
              <button className="clear-route-btn" onClick={() => { clearSearchMarker(); clearRoute(); setSearchQuery(""); setSearchResults([]); }}>
                ✕
              </button>
            </div>
          </div>
        ...
        <div className="route-banner">
          <div className="route-banner-info">
            ...
            <div>
              ...
              <span className="route-meta">
                {activeRoute.distanceKm} km &nbsp;·&nbsp; ~{activeRoute.durationMin} min by car
              </span>
            </div>
          </div>
          <div className="route-banner-actions">
            <button
              ...
              onClick={() => openExternalDirections(
                userLocation.lat, userLocation.lon,
                activeRoute.hospital.lat, activeRoute.hospital.lon,
                activeRoute.hospital.name
              )}
            >↗ Open in Google Maps</button>
            <button className="clear-route-btn" onClick={clearRoute}>✕ Clear</button>
          </div>
        </div>
      )}
      ...
      <div className="map-legend">
        <span><img src="https://raw.githubusercontent.com/pointhi/leaflet-color-markers/master/img/marker-icon-blue.png"   alt="you"      height="20" /> Your location</span>
        <span><img src="https://raw.githubusercontent.com/pointhi/leaflet-color-markers/master/img/marker-icon-red.png"    alt="hospital" height="20" /> Nearby hospital</span>
        <span><img src="https://raw.githubusercontent.com/pointhi/leaflet-color-markers/master/img/marker-icon-orange.png" alt="search"   height="20" /> Search result</span>
        <span><img src="https://raw.githubusercontent.com/pointhi/leaflet-color-markers/master/img/marker-icon-green.png"  alt="active"   height="20" /> Selected</span>
          ...
          <div className="hospitals-grid">
            {hospitals.map((h) => (
              <div
                ...
                onClick={() => { mapInstanceRef.current?.setView([h.lat, h.lon], 15); window.scrollTo({ top: 0, behavior: "smooth" }); }}
              >
                ...
                <div className="hospital-card-info">
                  ...
                  {h.phone && (
                    <a href={`tel:${h.phone}`} className="hospital-phone" onClick={(e) => e.stopPropagation()}>
                      📞 {h.phone}
                    </a>
                  )}
                  <div className="card-actions">
                    <button className="card-directions-btn" disabled={routeLoading} onClick={(e) => handleGetDirections(h, e)}>
                      {routeLoadingId === h.id ? "Loading..." : "🗺️ Get Directions"}
                    </button>
                    {userLocation && (
                      <button className="card-external-btn" onClick={(e) => { e.stopPropagation(); openExternalDirections(userLocation.lat, userLocation.lon, h.lat, h.lon, h.name); }}>
                        ↗ Maps
                      </button>
                    )}
                  </div>
